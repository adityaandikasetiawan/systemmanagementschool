...
exports.submitContact = async (req, res) => {
  try {
    ...
    try {
      await sendEmail({
        email: email,
        subject: 'Terima Kasih Telah Menghubungi Yayasan Baituljannah',
        html: emailTemplates.contactAutoReply({ name, subject, message })
      });
      ...
      await sendEmail({
        email: adminEmail,
        subject: `[Contact Form] Pesan Baru dari ${name} - ${unit_sekolah || 'Umum'}`,
        html: emailTemplates.contactMessageNotification({ 
          name, 
          email, 
          phone: phone || '-', 
          unit_sekolah: unit_sekolah || 'Umum', 
