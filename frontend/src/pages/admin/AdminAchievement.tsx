...
export const AdminAchievement: React.FC<AdminAchievementProps> = ({ onNavigate = () => { }, embedded = false }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing && currentAchievement) {
        const res = await api.achievements.update(currentAchievement.id, formData);
        if (res.success && res.data) {
          setAchievements(achievements.map(item =>
            item.id === currentAchievement.id ? res.data : item
          ));
        }
      } else {
        const res = await api.achievements.create(formData);
        if (res.success && res.data) {
  ...
  const handleDelete = async (id: number) => {
    if (confirm('Apakah Anda yakin ingin menghapus prestasi ini?')) {
      try {
        const res = await api.achievements.delete(id);
        if (res.success) {
              ...
              <tbody>
                {pagedAchievements.map((item, index) => (
                  <tr key={item.id} className={`border-b hover:bg-gray-50 transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-3 py-1 rounded-full text-xs ${item.rank.includes('1') ? 'bg-yellow-100 text-yellow-700' :
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-3 py-1 rounded-full text-xs ${item.status === 'published'
                  ...
                  <div className="flex gap-2">
                    {colors.map(color => (
                      <button
                        ...
                        className={`w-12 h-12 rounded-xl transition-all ${formData.accentColor === color.value
