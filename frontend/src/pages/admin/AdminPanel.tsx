...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const [activeSection, setActiveSection] = useState<'dashboard' | 'units' | 'users' | 'ppdb' | 'career' | 'finance' | 'library' | 'attendance' | 'students' | 'teachers' | 'classes' | 'materials' | 'assignments' | 'grades' | 'schedule' | 'profile' | 'content-news' | 'content-gallery' | 'content-achievement' | 'content-programs' | 'content-hero' | 'content-events'>('dashboard');
  React.useEffect(() => {
    try {
      ...
      return [
        ...
        { label: t('admin.menu.users'), icon: Users, section: 'users' },
        {
          ...
          submenu: [
            ...
            { label: 'Events/Kalender', onClick: () => setActiveSection('content-events') }
          ]
        },
  ...
  const renderContent = () => {
    switch (activeSection) {
      ...
      case 'content-events':
        return <AdminEvents onNavigate={onNavigate} embedded />;
      case 'students':
        ...
        {assignments.map((assignment) => (
          <div key={assignment.id} className="card hover:shadow-strong transition-shadow">
            <div className="flex items-start justify-between mb-4">
              ...
              <span className={`px-3 py-1 rounded-full text-xs ${assignment.status === 'Sudah Dikumpulkan' ? 'bg-green-100 text-green-700' :
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="md:col-span-2">
            ...
            <textarea
              className="input-field"
              rows={3}
              defaultValue="Jl. Pramuka No.43, Kemiling Permai, Kec. Kemiling, Kota Bandar Lampung, Lampung 35153"
            ></textarea>
          </div>
        </div>
